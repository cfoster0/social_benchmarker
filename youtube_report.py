...
def main():
	...
	with open(args.inputs, 'r') as csvfile:
		...
		for row in reader:
			if os.path.exists('results/youtube/' + row[1] + '.json'):
				genres.append(row[0])
				names.append(row[1])
	...
	for name, genrelist in zip(names, genres):
		with open('results/youtube/' + name + '.json') as data_file:
			...
			for post in data['Posts']:
				...
				theirengagements += post['dislikes']
				theirengagements += post['comments']
				theirengagements += post['favorites']
